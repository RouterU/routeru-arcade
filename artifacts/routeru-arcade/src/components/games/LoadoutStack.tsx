...
export default function LoadoutStack({
  ...
  useEffect(() => {
    ...
    const dropMs = isFastDropping ? FAST_DROP_MS : NORMAL_DROP_MS;
    const interval = window.setInterval(stepDown, dropMs);

    return () => window.clearInterval(interval);
  }, [isFastDropping, isRunning, stepDown]);

  useEffect(() => {
    if (mode !== "timed" || !isRunning) return;
      ...
      <div className="mx-auto max-w-6xl grid gap-4 lg:grid-cols-[320px_1fr_280px]">
        <div className="rounded-3xl bg-slate-900/90 border border-slate-700 p-4 shadow-2xl">
          ...
          <div className="grid gap-3">
            ...
            <StatCard
              label={mode === "survival" ? "Mode" : "Time Left"}
              value={mode === "survival" ? "Survival" : `${timeLeft ?? 0}s`}
            />
          </div>
        ...
        <div className="rounded-3xl bg-slate-900/90 border border-slate-700 p-4 shadow-2xl flex items-center justify-center">
          <div
            className="grid gap-1 bg-slate-950 p-3 rounded-2xl border border-slate-700"
            style={{
          ...
          >
            {renderBoard.flatMap((row, y) =>
              row.map((cell, x) => (
                <div
                  key={`${x}-${y}`}
                  className={`rounded-md border border-slate-800 ${cell ?? "bg-slate-900"}`}
                />
              ))
            )}
          </div>
